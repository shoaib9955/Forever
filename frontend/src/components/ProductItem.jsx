...
const ProductItem = ({ id, image, name, price, bestseller }) => {
  ...
  const product = {
    ...
    bestseller,
    description:
      products.find((p) => p._id === id)?.description ||
      "High-quality skincare product with premium ingredients for optimal results.",
    volume: products.find((p) => p._id === id)?.volume || [
      "30ml",
      "50ml",
      "100ml",
    ],
  ...
  return (
    <>
      <div className="group relative">
        ...
        >
          <div className="overflow-hidden rounded-xl shadow-sm group-hover:shadow-lg transition-shadow duration-300 relative">
            <img
              className="hover:scale-110 transition ease-in-out h-48 sm:h-56 md:h-64 lg:h-72 w-full object-cover border rounded-lg overflow-hidden shadow-sm"
              src={image[0]}
              alt={name}
            />
        ...
        <div className="mt-2">
          <p className="pt-3 pb-1 text-sm truncate group-hover:text-black transition-colors">
            {name}
          </p>
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium group-hover:text-black transition-colors">
              {currency}
              {price}
            </p>
            {bestseller && (
      ...
      {showComparison && (
        <ProductComparison
          products={compareProducts.map((id) => ({
            _id: id,
            name: "",
            price: 0,
            image: [""],
